...
SET search_path TO opencode, public;

-- The upstream opencode-database-plugin DDL is applied verbatim below.
-- The plugin's SQL is unqualified, so setting `search_path` to
-- `opencode, public` causes the plugin's tables, indexes, triggers,
-- functions, and views to be created under the dedicated `opencode`
-- schema without having to schema-qualify every statement.
--
-- The original upstream file is vendored at
-- `migrations/vendor/opencode-database-plugin/schema.sql` for
-- reproducibility and license-tracking. If the upstream schema is
-- updated, refresh the vendored copy and re-apply this migration in a
-- follow-up.
--
-- `gen_random_uuid()` is provided by core PostgreSQL 13+ in the
-- `public` schema, so pgcrypto is not required for these tables and
-- is not enabled here. (The plan called for a defensive
-- `CREATE EXTENSION IF NOT EXISTS pgcrypto;`, but pgcrypto is not
-- bundled in pglite-oxide, the embedded Postgres used by `cargo test`;
-- skipping the extension keeps the migration working in both the
-- dev container Postgres and the in-process test environment.)
