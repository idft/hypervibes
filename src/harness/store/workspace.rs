...
use super::common::{
    EventRunInsertMode, insert_run_with_model_variant_in_tx, lock_agent_coordination_tx,
...
/// The agent row is locked before the job row and all active-task checks.
pub async fn insert_analysis_coding_task_and_run(
    ...
    if trigger_mode == CodingTriggerMode::Automatic && !enabled {
        anyhow::bail!("analysis coding job is disabled")
    }
    ...
    if let Some(source_memory_id) = source_memory_id {
        let duplicate: Option<(i64,)> = query_as(
            "SELECT id FROM harness_maintenance_tasks
    ...
    if agent_has_blocking_workspace_maintenance_for_mode_tx(
        &mut tx,
        agent_key,
        EventRunInsertMode::CodingTrigger,
    )
    .await?
    {
        tx.rollback()
