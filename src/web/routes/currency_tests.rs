...
fn rejects_invalid_currency_logo_filenames() {
    for filename in [".svg", "BTC", "BTC.png", "BTC.svg.svg", "../BTC.svg", "BTC/ETH.svg"] {
        assert!(parse_currency_logo_filename(filename).is_err(), "{filename}");
    }
}
...
fn svg_validator_requires_safe_namespace_and_content() {
    ...
    for invalid in [
        ...
        br#"<svg xmlns="http://www.w3.org/2000/svg"><use href="https://evil.test/x"/></svg>"#.as_slice(),
    ] {
