...
- Read per-timeframe `analysis` memories with `vibetrading_get_latest_analysis` and write `market_analysis` memories with `vibetrading_write_memory`. For `market_analysis`, omit the `timeframe` argument entirely when calling `vibetrading_write_memory`. Never pass a placeholder such as `__omit__`, `none`, `null`, or an empty string; the backend rejects a timeframe on market-analysis memories.
